...
func Subscriber(input *SubscriberInput) {
	...
	ginkgo.It("should subscribe", func() {
		go func() {
			...
			Ω.Expect(subject.Subscribe(ctx, topic, handler, bps.StartAt(bps.Oldest))).To(Ω.Or(
				Ω.Succeed(),
	...
	ginkgo.It("should stop on bps.Done", func() {
		...
		Ω.Expect(subject.Subscribe(ctx, topic, handler, bps.StartAt(bps.Oldest))).To(Ω.Succeed())
	...
	ginkgo.It("should return handler error", func() {
		...
		actualErr := subject.Subscribe(ctx, topic, handler, bps.StartAt(bps.Oldest))
